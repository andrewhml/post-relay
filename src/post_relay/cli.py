...
def preferences_set(
    ...
    goal_type: Optional[str] = typer.Option(None, "--goal-type", help="Account goal type, such as growth, education, portfolio, or sales_leads."),
    growth_mode: Optional[str] = typer.Option(None, "--growth-mode", help="Growth posture: conservative, balanced, growth_push, or experimental."),
    primary_success_metric: Optional[str] = typer.Option(None, "--primary-success-metric", help="Primary optimization metric, such as followers, saves, shares, or comments."),
    target_monthly_reels: Optional[int] = typer.Option(None, "--target-monthly-reels", min=0, help="Optional target number of reels per month."),
    target_monthly_carousels: Optional[int] = typer.Option(None, "--target-monthly-carousels", min=0, help="Optional target number of carousels per month."),
    target_weekly_posts: Optional[int] = typer.Option(None, "--target-weekly-posts", min=0, help="Optional total post cadence target per week."),
    agent_checkin_cadence: Optional[str] = typer.Option(None, "--agent-checkin-cadence", help="Planned opt-in agent check-in cadence, such as weekly or off."),
    comfort_zone_push_enabled: bool = typer.Option(False, "--comfort-zone-push/--no-comfort-zone-push", help="Allow advisory recommendations to include bounded stretch moves."),
    max_push_level: Optional[str] = typer.Option(None, "--max-push-level", help="Maximum allowed comfort-zone push level: low, medium, or high."),
    preferred_growth_experiment: Optional[list[str]] = typer.Option(None, "--preferred-growth-experiment", help="Preferred growth experiment type."),
    blocked_growth_experiment: Optional[list[str]] = typer.Option(None, "--blocked-growth-experiment", help="Growth experiment type to avoid."),
    checkin_delivery_destination: Optional[str] = typer.Option(None, "--checkin-delivery-destination", help="Opt-in check-in destination preference: discord_dm, discord_home, whatsapp, or local_only."),
    checkin_trigger_policy: Optional[str] = typer.Option(None, "--checkin-trigger-policy", help="Check-in trigger policy: meaningful_only, meaningful_plus_weekly, weekly, or manual."),
    checkin_timezone: Optional[str] = typer.Option(None, "--checkin-timezone", help="Timezone for working-hours delivery, e.g. America/New_York."),
    checkin_working_hours_start: Optional[str] = typer.Option(None, "--checkin-working-hours-start", help="Working-hours start in HH:MM 24-hour format."),
    checkin_working_hours_end: Optional[str] = typer.Option(None, "--checkin-working-hours-end", help="Working-hours end in HH:MM 24-hour format."),
    checkin_run_planners: bool = typer.Option(False, "--checkin-run-planners/--no-checkin-run-planners", help="Allow read-only local planners to run before composing check-ins."),
    ...
    try:
        preferences = upsert_account_preferences(
            ...
            primary_success_metric=primary_success_metric,
            target_monthly_reels=target_monthly_reels,
            target_monthly_carousels=target_monthly_carousels,
            target_weekly_posts=target_weekly_posts,
            agent_checkin_cadence=agent_checkin_cadence,
            comfort_zone_push_enabled=comfort_zone_push_enabled,
            max_push_level=max_push_level,
            preferred_growth_experiments=preferred_growth_experiment or [],
            blocked_growth_experiments=blocked_growth_experiment or [],
            checkin_delivery_destination=checkin_delivery_destination,
            checkin_trigger_policy=checkin_trigger_policy,
            checkin_timezone=checkin_timezone,
            checkin_working_hours_start=checkin_working_hours_start,
            checkin_working_hours_end=checkin_working_hours_end,
            checkin_run_planners=checkin_run_planners,
            reviewed_by=reviewed_by,
            change_note=change_note,
        )
