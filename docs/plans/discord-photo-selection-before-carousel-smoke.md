...
The primary live Discord surface should be Andrew's private DM with the agent, not a public or semi-public review channel. The rollout should be DM-first and user-initiated-first: prove Andrew can start and complete a post conversation in DM, including selection and guided review, before adding agent-initiated suggestions. Later agent-initiated examples include new processed media detection, ideal posting cadence, inactivity since the last post, Andrew-provided life/trip context, holiday or culturally relevant moments, sensitive current events relevant to Andrew's travel profile goals, and trend/timing signals that suggest preparing a post before a deadline. See `docs/plans/discord-dm-conversation-orchestration.md` for the DM-first conversation plan and `docs/plans/postrelay-agent-operating-baseline.md` for the specialized content curator/social media manager baseline.
