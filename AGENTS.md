...
See `docs/plans/current-agent-roadmap.md`, `docs/plans/product-onboarding-roadmap.md`, `docs/plans/recommendation-engine-roadmap.md`, and `implementation-plan.md`. PR #84 / `feat/r2-setup-doctor` is merged, so self-managed BYO R2 onboarding now has no-network diagnostics for bucket, S3 endpoint URL, public base URL, required env values, and endpoint/public URL separation while redacting secrets. Andrew has paused the managed R2 staging direction for now; do not start `feat/managed-r2-staging-mvp` unless he explicitly reactivates that path. The next planning area should look for higher-leverage product opportunities that make the agent smarter and start building a true recommendation engine: better candidate ranking, fewer unnecessary questions, schedule/caption/post-type recommendations, use of approval/revision history, and local/read-only analytics or follower-growth signals. Keep all recommendation work local-first, auditable, dry-run/advisory by default, and free of Discord sends, R2 `--execute`, Meta publish `--execute`, or post lifecycle mutations unless a specific command and active-session authorization require them. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag. Do not publish another real post before the approved scheduled time unless Andrew explicitly bypasses the schedule in the active session.
