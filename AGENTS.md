...
See `docs/plans/current-agent-roadmap.md` and `docs/plans/product-onboarding-roadmap.md`. Post Relay is shifting from Andrew-only operation toward reusable local-first onboarding for friends and trusted beta users. Current PR #81 / branch `feat/meta-oauth-login` adds a trusted-tester OAuth helper that prints a Meta login URL, exchanges a copied callback code for a local user token, discovers visible Pages/linked IG accounts, and saves selected values to private `.env` only with explicit `--update-env`. PR #80 / `feat/meta-account-discovery`, PR #79 / `feat/setup-wizard`, PR #78 / `feat/setup-doctor`, and PR #77 / `docs/friend-onboarding-setup` are merged. The next milestone after OAuth login should be `docs/managed-staging-design`. Use `analytics feedback-summary` plus `analytics follower-summary` as advisory baselines when planning reviewed posts. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag. Do not publish another real post before the approved scheduled time unless Andrew explicitly bypasses the schedule in the active session.
