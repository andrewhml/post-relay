...
See `docs/plans/current-agent-roadmap.md`, `docs/plans/product-onboarding-roadmap.md`, `docs/plans/recommendation-engine-roadmap.md`, and `implementation-plan.md`. PR #84 / `feat/r2-setup-doctor` is merged, so self-managed BYO R2 onboarding now has no-network diagnostics for bucket, S3 endpoint URL, public base URL, required env values, and endpoint/public URL separation while redacting secrets. Andrew has paused the managed R2 staging direction for now; do not start `feat/managed-r2-staging-mvp` unless he explicitly reactivates that path. PR #86 / `feat/user-goal-artifact`, PR #87 / `feat/chat-goal-onboarding`, PR #88 / `feat/recommendation-signal-baseline`, PR #89 / `feat/candidate-ranking-signals`, PR #90 / `feat/smarter-context-questions`, and PR #91 / `fix/meta-location-tag-choice` are merged, so active user/agent goals, local recommendation signal coverage, deterministic candidate ranking, smarter context-question reduction, and location-tag publish safeguards exist. The next milestone is `feat/schedule-recommendations`: add local/no-network schedule-window suggestions that read the active goal, scheduled queue, stored post/publish history, and sparse analytics/follower signals before recommending a slot. Keep this milestone advisory and free of Discord sends, R2 `--execute`, Meta publish `--execute`, automatic scheduling, approval mutation, post lifecycle mutation, or publish attempts. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag; a visible Instagram location requires a reviewed `location_id` or an explicit skip/bypass before publish.
