...
See `docs/plans/current-agent-roadmap.md` and `docs/plans/product-onboarding-roadmap.md`. Post Relay is shifting from Andrew-only operation toward reusable local-first onboarding for friends and trusted beta users. Current PR #82 / branch `docs/managed-staging-design` designs managed R2 staging with invite/user auth, server-owned R2 credentials, presigned direct uploads, opaque object keys, TTL cleanup, quotas, privacy copy, rollback/failure modes, and no implementation-side live upload behavior. PR #81 / `feat/meta-oauth-login`, PR #80 / `feat/meta-account-discovery`, PR #79 / `feat/setup-wizard`, PR #78 / `feat/setup-doctor`, and PR #77 / `docs/friend-onboarding-setup` are merged. The next milestone after managed staging design should be `feat/managed-r2-staging-mvp` only after the design is reviewed. Use `analytics feedback-summary` plus `analytics follower-summary` as advisory baselines when planning reviewed posts. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag. Do not publish another real post before the approved scheduled time unless Andrew explicitly bypasses the schedule in the active session.
