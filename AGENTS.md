...
See `docs/plans/current-agent-roadmap.md` and `docs/plans/product-onboarding-roadmap.md`. Post Relay is shifting from Andrew-only operation toward reusable local-first onboarding for friends and trusted beta users. Current PR #78 / branch `feat/setup-doctor` adds a no-network diagnostic command for local config, DB, photo-root, artifact/export-root, and optional integration readiness. PR #77 / `docs/friend-onboarding-setup` is merged and established generic local preview onboarding. The next milestone after setup doctor should be `feat/setup-wizard`, a non-destructive guided local setup command. Use `analytics feedback-summary` plus `analytics follower-summary` as advisory baselines when planning reviewed posts. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag. Do not publish another real post before the approved scheduled time unless Andrew explicitly bypasses the schedule in the active session.
