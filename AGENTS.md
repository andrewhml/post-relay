...
See `docs/plans/current-agent-roadmap.md` and `docs/plans/product-onboarding-roadmap.md`. Post Relay is shifting from Andrew-only operation toward reusable local-first onboarding for friends and trusted beta users. Current PR #77 / branch `docs/friend-onboarding-setup` is documentation/template-only: rewrite generic onboarding around local preview mode, add `docs/setup-own-instance.md`, generalize `.env.example` and `config/photo_sources.example.yaml`, and preserve Andrew-specific live instance facts only in agent/operations handoff surfaces. The next code milestone after this docs PR should be `feat/setup-doctor`, a no-network diagnostic command for local config/DB/photo-root/integration readiness. Use `analytics feedback-summary` plus `analytics follower-summary` as advisory baselines when planning reviewed posts. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag. Do not publish another real post before the approved scheduled time unless Andrew explicitly bypasses the schedule in the active session.
