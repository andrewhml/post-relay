...
See `docs/plans/current-agent-roadmap.md` and `docs/plans/product-onboarding-roadmap.md`. Post Relay is shifting from Andrew-only operation toward reusable local-first onboarding for friends and trusted beta users. Current PR #83 / branch `docs/byo-r2-friend-setup` documents bring-your-own Cloudflare R2 bucket as the first-round staging path for technical beta users, including the exact Cloudflare credential path for S3-compatible R2 Access Key ID / Secret Access Key creation. PR #82 / `docs/managed-staging-design`, PR #81 / `feat/meta-oauth-login`, PR #80 / `feat/meta-account-discovery`, PR #79 / `feat/setup-wizard`, PR #78 / `feat/setup-doctor`, and PR #77 / `docs/friend-onboarding-setup` are merged. The next milestone after BYO R2 docs should be `feat/r2-setup-doctor`; managed staging remains a later convenience layer after self-managed friend setup is proven. Use `analytics feedback-summary` plus `analytics follower-summary` as advisory baselines when planning reviewed posts. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag. Do not publish another real post before the approved scheduled time unless Andrew explicitly bypasses the schedule in the active session.
