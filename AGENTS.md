...
See `docs/plans/current-agent-roadmap.md`, `docs/plans/product-onboarding-roadmap.md`, `docs/plans/recommendation-engine-roadmap.md`, and `implementation-plan.md`. PR #84 / `feat/r2-setup-doctor` is merged, so self-managed BYO R2 onboarding now has no-network diagnostics for bucket, S3 endpoint URL, public base URL, required env values, and endpoint/public URL separation while redacting secrets. Andrew has paused the managed R2 staging direction for now; do not start `feat/managed-r2-staging-mvp` unless he explicitly reactivates that path. PR #86 / `feat/user-goal-artifact`, PR #87 / `feat/chat-goal-onboarding`, and PR #88 / `feat/recommendation-signal-baseline` are merged, so active user/agent goals and local recommendation signal coverage exist. The active milestone is PR #89 / `feat/candidate-ranking-signals`: add a no-network `recommendations candidates` command that ranks candidate groups with deterministic explainable local scoring from readiness, content potential, active-goal alignment, effort, and sparse-learning signals. Keep all recommendation work local-first, auditable, dry-run/advisory by default, and free of Discord sends, R2 `--execute`, Meta publish `--execute`, or post lifecycle mutations unless a specific command and active-session authorization require them. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag. Do not publish another real post before the approved scheduled time unless Andrew explicitly bypasses the schedule in the active session.
