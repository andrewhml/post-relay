...
See `docs/plans/current-agent-roadmap.md` and `docs/plans/product-onboarding-roadmap.md`. Post Relay is shifting from Andrew-only operation toward reusable local-first onboarding for friends and trusted beta users. Current PR #80 / branch `feat/meta-account-discovery` adds a read-only helper to discover visible Facebook Pages and linked Instagram accounts from a private token, with explicit non-secret `.env` account-ID updates. PR #79 / `feat/setup-wizard`, PR #78 / `feat/setup-doctor`, and PR #77 / `docs/friend-onboarding-setup` are merged. The next milestone after account discovery should be `feat/meta-oauth-login`, a trusted-tester OAuth helper that keeps tokens local. Use `analytics feedback-summary` plus `analytics follower-summary` as advisory baselines when planning reviewed posts. Freeform `location_text` remains local/review-only and must not be sent as a Meta location tag. Do not publish another real post before the approved scheduled time unless Andrew explicitly bypasses the schedule in the active session.
